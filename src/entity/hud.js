    ...
    render() {
        ...
        ctx.wrap(() => {
            ...
            for (const [label, value] of [
                [nomangle('关卡'), (G.runLevelIndex) + '/' + (ALL_LEVELS.length - 1)],
                [nomangle('时间'), formatTime(G.runTime)],
                [nomangle('最佳'), formatTime(G.bestRunTime)],
                [nomangle('死亡'), G.runDeaths],
                [nomangle('难度[K]'), G.difficulty.label],
            ]) {
